...
next
  ...
  next
    ...
    have 2: "x ##\<^sub>S \<Squnion>\<^sub>S (filter (\<lambda>x. x \<notin> set ys) xs)"
      by (metis (no_types, lifting) Cons.prems(1) filter_is_subset list.simps(15) pairwise_compat_foldr pairwise_insert pairwise_subset scene_compat_refl subset_code(1))
